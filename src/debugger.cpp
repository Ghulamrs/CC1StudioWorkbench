...
// And it is asked for in two halves, which is not decoration. lldb echoes each
// command it is given when its input is a pipe, so a command with the marker
// written in it puts the marker on the stream before the answer rather than
// after - and every answer read that way is the one before the one asked for.
// Joined where it is printed and nowhere else, the echo cannot be mistaken for
// the reply.
...
std::vector<Variable> Debugger::locals() {
    ...
    return readVariables(kind_, ask(kind_ == DebuggerGdb ? "info locals" : "frame variable"));
}
